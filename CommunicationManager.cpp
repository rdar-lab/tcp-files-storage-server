...
void CommunicationManager::runServer(std::string host, unsigned int port) {
	...
	boost::asio::ip::address address = boost::asio::ip::address_v4::from_string(host);
	...
	for (;;)
	{
		if (this->numOfConnections >= MAX_NUMBER_OF_TCP_CONNECTIONS){
			std::cout << "Max number of connections reached, waiting..." << std::endl;
...
void CommunicationManager::handleClient(socket_ptr socket) {
	std::string remote_address = socket->remote_endpoint().address().to_string() + ":" + std::to_string(socket->remote_endpoint().port());
	...
	try {
		try{
			...
			std::cout << remote_address <<  ": Handling request: " << *req << std::endl;
			RequestHandler* handler = RequestHandlerFactory::getInstance()->getHandlerForRequest(req);
			if (handler == NULL){
			...
			std::cerr << remote_address << ": Exception in handling request: " << exp.what() << std::endl;
			resp = new Response(GENERAL_FAILURE_STATUS);
		}

		std::cout << remote_address <<  ": Sending response: " << *resp << std::endl;
		ResponseWriter writer(&(*socket));
		writer.writeResponse(resp);
	} catch (std::exception &exp){
		std::cerr << remote_address << ": Exception  in client thread: " << exp.what() << std::endl;
	}
